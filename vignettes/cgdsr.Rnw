...
A verbose option can be set for the CGDS connection object. This will cause function calls that retrieve data from cBioPortal to additionally display the programming interface URL to be displayed. This is useful for debugging and troubleshooting issues with the package. 
